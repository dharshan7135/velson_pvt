...
import { ListPlus, Database, Package } from 'lucide-react';
const columns = [{ key: 'Model_No', label: 'Model No' },{ key: 'Vehicle_Model_Name', label: 'Vehicle' },{ key: 'Assembly_Part_Name', label: 'Assembly' },{ key: 'Qty', label: 'Qty' },{ key: 'Status', label: 'Status' }];
const emptyForm = {
...
const IndexCreation = () => {
    ...
    const save = () => { if (editId) updateRecord('indexCreation', editId, form); else addRecord('indexCreation', form); close(); };
    return (
        <div className="p-6">
            <PageHeader title="Index Creation" description="Assembly part index with vehicle model linkage" icon={ListPlus} />
            <DataTable columns={columns} data={state.indexCreation || []} onAdd={openAdd} addLabel="New Index" onEdit={openEdit} onDelete={(r) => deleteRecord('indexCreation', r.id)} />
            <FormModal isOpen={modal} onClose={close} title={editId ? 'Edit' : 'New Index Entry'} size="lg">
                <div className="p-6 space-y-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <FormField label="Assembly Part Name" id="Assembly_Part_Name" value={form.Assembly_Part_Name} onChange={(e) => set('Assembly_Part_Name', e.target.value)} required />
