...
import { Banknote, Database, Calendar, DollarSign, Shield, Settings, AlignLeft } from 'lucide-react';
...
const PaymentMaster1 = () => {
    ...
    const save = () => { if (editId) updateRecord('paymentMaster1', editId, form); else addRecord('paymentMaster1', form); close(); };

    return (
        <div className="p-6">
            <PageHeader title="Payment Master" description="Payment voucher management with cheque, bank and collection details" icon={Banknote} />
            <DataTable columns={columns} data={state.paymentMaster1 || []} onAdd={openAdd} addLabel="New Payment" onEdit={openEdit} onDelete={(r) => deleteRecord('paymentMaster1', r.id)} />
            <FormModal isOpen={modal} onClose={close} title={editId ? 'Edit Payment' : 'New Payment'} size="full">
