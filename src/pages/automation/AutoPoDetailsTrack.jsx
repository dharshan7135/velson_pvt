...
import { History, Database, Package, DollarSign, Shield } from 'lucide-react';
...
const emptyForm = {
    ...
    Qty: 0, Closing_Qty: 0, Re_Order_Qty: 0, Po_Qty: 0,
    Job_Qty: 0, poqty: 0, Autoqty: 0, stockqty: 0, GRN_Qty: 0,
    Order_Type: '',
    Po_Status: '', Status: 'A',
...
const AutoPoDetailsTrack = () => {
    ...
    const save = () => { if (editId) updateRecord('autoPoDetailsTrack', editId, form); else addRecord('autoPoDetailsTrack', form); close(); };

    return (
        <div className="p-6">
            <PageHeader title="Auto PO Details Track" description="Track history of automated purchase order details" icon={History} />
            <DataTable columns={columns} data={state.autoPoDetailsTrack || []} onAdd={openAdd} addLabel="New Track Entry" onEdit={openEdit} onDelete={(r) => deleteRecord('autoPoDetailsTrack', r.id)} />
            <FormModal isOpen={modal} onClose={close} title={editId ? 'Edit Track Entry' : 'New Track Entry'} size="full">
                <div className="p-6 space-y-6">
                    ...
                    <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
                        <FormContainer title="Quantities" icon={Package}>
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                ...
                                <FormField label="PO Order Qty" id="poqty" type="number" value={form.poqty} onChange={(e) => set('poqty', e.target.value)} />
                    ...
                    <div className="flex justify-end gap-3 pt-4 border-t">
                        ...
                        <button onClick={save} className="px-5 py-2.5 bg-[#0097A7] text-white rounded-xl font-semibold text-sm hover:bg-[#00838F] transition-colors cursor-pointer">{editId ? 'Update' : 'Save Track Entry'}</button>
                    </div>
                </div>
            </FormModal>
        </div>
...
export default AutoPoDetailsTrack;
