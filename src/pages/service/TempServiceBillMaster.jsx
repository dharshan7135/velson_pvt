...
import { FileInput, Database, DollarSign, Users, Shield, Truck, MapPin } from 'lucide-react';
...
const TempServiceBillMaster = () => {
    ...
    const save = () => { if (editId) updateRecord('tempServiceBillMasters', editId, form); else addRecord('tempServiceBillMasters', form); close(); };

    return (
        <div className="p-6">
            <PageHeader title="Temp Service Bill Master" description="Staging area for service bills before finalization" icon={FileInput} />
            <DataTable columns={columns} data={state.tempServiceBillMasters || []} onAdd={openAdd} addLabel="New Temp Bill" onEdit={openEdit} onDelete={(r) => deleteRecord('tempServiceBillMasters', r.id)} />
            <FormModal isOpen={modal} onClose={close} title={editId ? 'Edit Temp Bill' : 'New Temp Service Bill'} size="full">
                <div className="p-6 space-y-6">
                    <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
                        <FormContainer title="Bill Header" icon={Database}>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <FormField label="Booking Cust Code" id="Booking_Cus_Code" value={form.Booking_Cus_Code} onChange={(e) => set('Booking_Cus_Code', e.target.value)} />
                    ...
                    <div className="flex justify-end gap-3 pt-4 border-t">
                        ...
                        <button onClick={save} className="px-5 py-2.5 bg-[#0097A7] text-white rounded-xl font-semibold text-sm hover:bg-[#00838F] transition-colors cursor-pointer">{editId ? 'Update' : 'Save Temp Bill'}</button>
                    </div>
                </div>
            </FormModal>
        </div>
...
export default TempServiceBillMaster;
