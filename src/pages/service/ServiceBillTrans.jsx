...
import { ArrowRightLeft, Database, DollarSign, Package, Shield } from 'lucide-react';
...
const ServiceBillTrans = () => {
    ...
    const save = () => { if (editId) updateRecord('serviceBillTrans', editId, form); else addRecord('serviceBillTrans', form); close(); };

    return (
        <div className="p-6">
            <PageHeader title="Service Bill Transaction" description="Line items for service bills with pricing and tax" icon={ArrowRightLeft} />
            <DataTable columns={columns} data={state.serviceBillTrans || []} onAdd={openAdd} addLabel="Add Line" onEdit={openEdit} onDelete={(r) => deleteRecord('serviceBillTrans', r.id)} />
            <FormModal isOpen={modal} onClose={close} title={editId ? 'Edit Transaction' : 'New Bill Transaction'} size="full">
...
export default ServiceBillTrans;
