...
const columns = [{ key: 'DC_No', label: 'DC No' },{ key: 'Part_Name', label: 'Part' },{ key: 'Qty', label: 'Qty' },{ key: 'Amount', label: 'Amt' },{ key: 'Status', label: 'Status' }];
const emptyForm = {
    Row_id: '', Item_Id: '', DC_No: '', Part_No: '', Part_Name: '',
...
const DCDetails = () => {
    ...
    const save = () => { if (editId) updateRecord('dcDetails', editId, form); else addRecord('dcDetails', form); close(); };
    return (
        <div className="p-6">
            <PageHeader title="DC Details" description="Dispatch challan line items with barcode and material source" icon={ListOrdered} />
            <DataTable columns={columns} data={state.dcDetails || []} onAdd={openAdd} addLabel="Add Line" onEdit={openEdit} onDelete={(r) => deleteRecord('dcDetails', r.id)} />
            <FormModal isOpen={modal} onClose={close} title={editId ? 'Edit' : 'New DC Line'} size="lg">
                <div className="p-6 space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <FormField label="HRC" id="HRC" value={form.HRC} onChange={(e) => set('HRC', e.target.value)} />
