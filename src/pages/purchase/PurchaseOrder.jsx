...
import { ShoppingCart, Database, DollarSign, Truck, Calendar, Settings, Shield } from 'lucide-react';
...
const emptyForm = {
    ...
    PO_No: '', Contact_No: '', Purchase_Req_No: '', PO_Type: '', AutoPo_Type: '', Grade: '',
    ...
    Project: '', Destination: '', delivery_period: '', Warrenty_terms: '',
...
const PurchaseOrder = () => {
    ...
    const save = () => { if (editId) updateRecord('purchaseOrders', editId, form); else addRecord('purchaseOrders', form); close(); };

    return (
        <div className="p-6">
            <PageHeader title="Purchase Order" description="Create and manage purchase orders with supplier and delivery terms" icon={ShoppingCart} />
            <DataTable columns={columns} data={state.purchaseOrders || []} onAdd={openAdd} addLabel="New PO" onEdit={openEdit} onDelete={(r) => deleteRecord('purchaseOrders', r.id)} />
            <FormModal isOpen={modal} onClose={close} title={editId ? 'Edit PO' : 'New Purchase Order'} size="full">
                <div className="p-6 space-y-6">
                    <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
                        ...
                        <FormContainer title="Delivery & Terms" icon={Truck}>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <FormField label="Delivery Period" id="delivery_period" value={form.delivery_period} onChange={(e) => set('delivery_period', e.target.value)} />
                    ...
                    <div className="flex justify-end gap-3 pt-4 border-t">
                        ...
                        <button onClick={save} className="px-5 py-2.5 bg-[#0097A7] text-white rounded-xl font-semibold text-sm hover:bg-[#00838F] transition-colors cursor-pointer">{editId ? 'Update' : 'Save PO'}</button>
                    </div>
                </div>
            </FormModal>
        </div>
