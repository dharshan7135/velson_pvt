...
import { History, Database, DollarSign, Package, Shield, Barcode } from 'lucide-react';
...
const emptyForm = {
    ...
    GRN_Date: '', PO_Date: '', ETA_Date: '', Invoice_Date: '', deleted_date: '', Created_Date: new Date().toISOString().slice(0, 10),
...
const GRNEntryDetailsTrack = () => {
    ...
    const save = () => { if (editId) updateRecord('grnEntryDetailsTrack', editId, form); else addRecord('grnEntryDetailsTrack', form); close(); };

    return (
        <div className="p-6">
            <PageHeader title="GRN Details Track" description="Tracked GRN line items with audit trail for deletions and changes" icon={History} />
            <DataTable columns={columns} data={state.grnEntryDetailsTrack || []} onAdd={openAdd} addLabel="Add Track Line" onEdit={openEdit} onDelete={(r) => deleteRecord('grnEntryDetailsTrack', r.id)} />
            <FormModal isOpen={modal} onClose={close} title={editId ? 'Edit Track Line' : 'New Tracked GRN Detail'} size="full">
                    ...
                    <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
                        ...
                        <FormContainer title="Quantity & QC" icon={Shield}>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <FormField label="Track Details" id="Track_details" className="md:col-span-2" value={form.Track_details} onChange={(e) => set('Track_details', e.target.value)} />
                            </div>
                        </FormContainer>
                    </div>
...
export default GRNEntryDetailsTrack;
