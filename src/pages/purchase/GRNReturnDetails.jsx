...
import { RotateCcw, Database, DollarSign, Package, Shield, Barcode } from 'lucide-react';
...
const GRNReturnDetails = () => {
    ...
    const save = () => { if (editId) updateRecord('grnReturnDetails', editId, form); else addRecord('grnReturnDetails', form); close(); };

    return (
        <div className="p-6">
            <PageHeader title="GRN Return Details" description="Goods returned to supplier with QC rejection tracking" icon={RotateCcw} />
            <DataTable columns={columns} data={state.grnReturnDetails || []} onAdd={openAdd} addLabel="Add Return" onEdit={openEdit} onDelete={(r) => deleteRecord('grnReturnDetails', r.id)} />
            <FormModal isOpen={modal} onClose={close} title={editId ? 'Edit Return' : 'New GRN Return'} size="full">
                <div className="p-6 space-y-6">
                    <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
                        <FormContainer title="Item & GRN Ref" icon={Package}>
                        ...
                        <FormContainer title="Return Qty & QC" icon={Shield}>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <FormField label="Returned Qty" id="Qty" type="number" value={form.Qty} onChange={(e) => set('Qty', e.target.value)} required />
                                ...
                                <FormField label="QC Remarks" id="QC_Remarks" value={form.QC_Remarks} onChange={(e) => set('QC_Remarks', e.target.value)} />
                            </div>
                        </FormContainer>
                    </div>
                    ...
                    <div className="flex justify-end gap-3 pt-4 border-t">
                        ...
                        <button onClick={save} className="px-5 py-2.5 bg-[#0097A7] text-white rounded-xl font-semibold text-sm hover:bg-[#00838F] transition-colors cursor-pointer">{editId ? 'Update' : 'Save Return'}</button>
                    </div>
                </div>
            </FormModal>
        </div>
