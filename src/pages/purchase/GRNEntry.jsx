...
import { PackageCheck, Database, DollarSign, Truck, Settings, Shield, Building2 } from 'lucide-react';
...
const emptyForm = {
    ...
    GRN_Date: '', Invoice_Date: '', PO_Date: '', Delivery_Date: '', Created_Date: new Date().toISOString().slice(0, 10),
...
const GRNEntry = () => {
    ...
    const save = () => { if (editId) updateRecord('grnEntries', editId, form); else addRecord('grnEntries', form); close(); };

    return (
        <div className="p-6">
            <PageHeader title="GRN Entry" description="Goods Receipt Note header with supplier, PO, and ledger linkage" icon={PackageCheck} />
            <DataTable columns={columns} data={state.grnEntries || []} onAdd={openAdd} addLabel="New GRN" onEdit={openEdit} onDelete={(r) => deleteRecord('grnEntries', r.id)} />
            <FormModal isOpen={modal} onClose={close} title={editId ? 'Edit GRN' : 'New GRN Entry'} size="full">
                <div className="p-6 space-y-6">
                    <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
                        <FormContainer title="GRN Header" icon={Database}>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <FormField label="Inward Type" id="Inward_Type" value={form.Inward_Type} onChange={(e) => set('Inward_Type', e.target.value)} />
                            </div>
                        </FormContainer>
                        <FormContainer title="Supplier Details" icon={Building2}>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <FormContainer title="Financials" icon={DollarSign}>
                        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                            ...
                            <FormField label="Tax Amt" id="Tax_Amt" type="number" value={form.Tax_Amt} onChange={(e) => set('Tax_Amt', e.target.value)} />
                    ...
                    <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
                        <FormContainer title="Terms" icon={Settings}>
                    ...
                    <div className="flex justify-end gap-3 pt-4 border-t">
                        ...
                        <button onClick={save} className="px-5 py-2.5 bg-[#0097A7] text-white rounded-xl font-semibold text-sm hover:bg-[#00838F] transition-colors cursor-pointer">{editId ? 'Update' : 'Save GRN'}</button>
                    </div>
                </div>
            </FormModal>
        </div>
