...
import { FileOutput, Database, ShoppingBag, DollarSign, Percent, Calculator, Info } from 'lucide-react';
...
const emptyForm = {
    ...
    BILLNO: '', Batch: '', DC_Invoice_No: '', BILL_MODE: 'Credit', STATUS: 'A', DC_Status: 'Open', Entry_Model: '', BILLDATE: '',
    Item_name: '', HSNCode: '', COMMODITY: '', MRP: 0, MfrDate: '', ExpiryDate: '', Barcode: '',
    ...
    LIABILITY_VALUE: 0, TOT_AMT: 0, TAXABLE_AMT: 0, NET_AMT: 0,
...
const DCSalesTran = () => {
    ...
    const save = () => { if (editId) updateRecord('dcSalesTrans', editId, form); else addRecord('dcSalesTrans', form); close(); };

    return (
        <div className="p-6">
            <PageHeader title="DC Sales Transaction" description="Line-level items on Delivery Challans with invoice mapping" icon={FileOutput} />
            <DataTable columns={columns} data={state.dcSalesTrans || []} onAdd={openAdd} addLabel="Add DC Line" onEdit={openEdit} onDelete={(r) => deleteRecord('dcSalesTrans', r.id)} />
            <FormModal isOpen={modal} onClose={close} title={editId ? 'Edit DC Line' : 'New DC Transaction Line'} size="full">
                <div className="p-6 space-y-6">
                    <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
                        <FormContainer title="DC Transaction Header" icon={Database}>
                        ...
                        <FormContainer title="Item Details" icon={ShoppingBag}>
                    ...
                    <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
                        <FormContainer title="Quantity" icon={Calculator}>
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                ...
                                <FormField label="Unit Qty" id="Unit_Qty" type="number" value={form.Unit_Qty} onChange={(e) => set('Unit_Qty', e.target.value)} />
                                <FormField label="Unit Qty 1" id="Unit_Qty1" type="number" value={form.Unit_Qty1} onChange={(e) => set('Unit_Qty1', e.target.value)} />
                                <FormField label="Bag" id="BAG" value={form.BAG} onChange={(e) => set('BAG', e.target.value)} />
                            </div>
                        </FormContainer>
                        <FormContainer title="Pricing" icon={DollarSign}>
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                ...
                                <FormField label="Resale Rate" id="RESALE_RATE" type="number" value={form.RESALE_RATE} onChange={(e) => set('RESALE_RATE', e.target.value)} />
                                <FormField label="Cost Rate" id="Cost_Rate" type="number" value={form.Cost_Rate} onChange={(e) => set('Cost_Rate', e.target.value)} />
                                <FormField label="Rate By" id="Rateby" type="number" value={form.Rateby} onChange={(e) => set('Rateby', e.target.value)} />
                            </div>
                        </FormContainer>
                    </div>
                    <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
                        <FormContainer title="Discounts" icon={Percent}>
                            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                                <FormField label="Disc Type" id="DISC_TYPE" value={form.DISC_TYPE} onChange={(e) => set('DISC_TYPE', e.target.value)} />
                                ...
                                <FormField label="Cust Free" id="CUS_FREE" type="number" value={form.CUS_FREE} onChange={(e) => set('CUS_FREE', e.target.value)} />
                                <FormField label="Soc Free" id="SOC_FREE" type="number" value={form.SOC_FREE} onChange={(e) => set('SOC_FREE', e.target.value)} />
                            </div>
                        </FormContainer>
                        <FormContainer title="Totals">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <FormField label="Tot Amt" id="TOT_AMT" type="number" value={form.TOT_AMT} onChange={(e) => set('TOT_AMT', e.target.value)} required />
                                <FormField label="Taxable" id="TAXABLE_AMT" type="number" value={form.TAXABLE_AMT} onChange={(e) => set('TAXABLE_AMT', e.target.value)} required />
                                <FormField label="Liability" id="LIABILITY_VALUE" type="number" value={form.LIABILITY_VALUE} onChange={(e) => set('LIABILITY_VALUE', e.target.value)} />
                                <FormField label="Net Amt" id="NET_AMT" type="number" className="font-bold text-[#0097A7] bg-cyan-50" value={form.NET_AMT} onChange={(e) => set('NET_AMT', e.target.value)} required />
                            </div>
                        </FormContainer>
                    </div>
                    <FormContainer title="Tax Config">
                        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                            <FormField label="Tax %" id="TAX_PER" type="number" value={form.TAX_PER} onChange={(e) => set('TAX_PER', e.target.value)} />
                            <FormField label="CGST %" id="CGST_TAX" type="number" value={form.CGST_TAX} onChange={(e) => set('CGST_TAX', e.target.value)} />
                            <FormField label="SGST %" id="SGST_TAX" type="number" value={form.SGST_TAX} onChange={(e) => set('SGST_TAX', e.target.value)} />
                            <FormField label="IGST %" id="IGST_TAX" type="number" value={form.IGST_TAX} onChange={(e) => set('IGST_TAX', e.target.value)} />
                    ...
                    <FormContainer title="Audit" icon={Info}>
                        <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
                            ...
                            <FormField label="S-Ledger" id="SLEDGER_ID" value={form.SLEDGER_ID} onChange={(e) => set('SLEDGER_ID', e.target.value)} />
                    ...
                    <div className="flex justify-end gap-3 pt-4 border-t">
                        ...
                        <button onClick={save} className="px-5 py-2.5 bg-[#0097A7] text-white rounded-xl font-semibold text-sm hover:bg-[#00838F] transition-colors cursor-pointer">{editId ? 'Update' : 'Save DC Line'}</button>
                    </div>
                </div>
            </FormModal>
        </div>
