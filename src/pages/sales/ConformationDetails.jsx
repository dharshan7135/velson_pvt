...
import { List, Database, Wrench, Package, Calendar } from 'lucide-react';
...
const ConformationDetails = () => {
    ...
    const save = () => { if (editId) updateRecord('conformationDetails', editId, form); else addRecord('conformationDetails', form); close(); };

    return (
        <div className="p-6">
            <PageHeader title="Confirmation Details" description="Service line items, spare parts, and problem tracking" icon={List} />
            <DataTable columns={columns} data={state.conformationDetails || []} onAdd={openAdd} addLabel="Add Detail Line" onEdit={openEdit} onDelete={(r) => deleteRecord('conformationDetails', r.id)} />
            <FormModal isOpen={modal} onClose={close} title={editId ? 'Edit Detail' : 'New Confirmation Detail'} size="xl">
...
export default ConformationDetails;
