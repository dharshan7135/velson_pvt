...
import { FileOutput, User, Truck, DollarSign, Settings, MapPin, Building2, Package } from 'lucide-react';

const columns = [
    { key: 'BILLNO', label: 'DC Bill No' },
...
const emptyForm = {
    ...
    BILLNO: '', Order_No: '', Dc_NO: '', LR_NO: '', BILL_MODE: 'Credit', STATUS: 'A', DC_Status: 'Open', Entry_Model: '', Stock_Reduce: '',
...
const DCSalesMaster = () => {
    ...
    const save = () => { if (editId) updateRecord('dcSalesMasters', editId, form); else addRecord('dcSalesMasters', form); close(); };

    return (
        <div className="p-6">
            <PageHeader title="DC Sales Master" description="Delivery Challan header for goods dispatched without invoice" icon={FileOutput} />
            <DataTable columns={columns} data={state.dcSalesMasters || []} onAdd={openAdd} addLabel="New DC Sales" onEdit={openEdit} onDelete={(r) => deleteRecord('dcSalesMasters', r.id)} />
            <FormModal isOpen={modal} onClose={close} title={editId ? 'Edit DC Sales' : 'New DC Sales Header'} size="full">
                <div className="p-6 space-y-6">
                    <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
                        <FormContainer title="DC Document Header" icon={Building2}>
                        ...
                        <FormContainer title="DC & Stock Config" icon={Package}>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
                        <FormContainer title="Bill-to Customer" icon={User}>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <FormContainer title="Ship-to Destination" icon={MapPin}>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <FormField label="Delivery Name" id="DCUST_NAME" value={form.DCUST_NAME} onChange={(e) => set('DCUST_NAME', e.target.value)} required />
                                <FormField label="Delivery Phone" id="DPHONE" value={form.DPHONE} onChange={(e) => set('DPHONE', e.target.value)} />
                    ...
                    <FormContainer title="DC Financial Summary" icon={DollarSign}>
                    ...
                    <div className="flex justify-end gap-3 pt-4 border-t">
                        ...
                        <button onClick={save} className="px-5 py-2.5 bg-[#0097A7] text-white rounded-xl font-semibold text-sm hover:bg-[#00838F] transition-colors cursor-pointer">{editId ? 'Update' : 'Save DC'}</button>
                    </div>
                </div>
            </FormModal>
        </div>
