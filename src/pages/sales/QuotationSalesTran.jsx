...
import { Activity, Database, ShoppingBag, DollarSign, Percent, Calculator, Info, Calendar, Barcode } from 'lucide-react';
...
const emptyForm = {
    ...
    // Document Details
    BILLNO: '', Batch: '', BILL_MODE: 'Credit', STATUS: 'A', BILLDATE: '', Entry_Model: '',
    // Item Details
...
const QuotationSalesTran = () => {
    ...
    const save = () => {
        if (editId) updateRecord('quotationSalesTrans', editId, form);
        else addRecord('quotationSalesTrans', form);
        close();
    };

    return (
        <div className="p-6">
            <PageHeader title="Sales Transaction Line" description="Manage line-level sales transactions, taxes, and barcode tracking" icon={Activity} />
            
            <DataTable 
                columns={columns} 
                data={state.quotationSalesTrans || []} 
                onAdd={openAdd} 
                addLabel="Add Sales Line" 
                onEdit={openEdit} 
                onDelete={(r) => deleteRecord('quotationSalesTrans', r.id)} 
            />
            ...
            <FormModal isOpen={modal} onClose={close} title={editId ? 'Edit Sales Line' : 'New Sales Line'} size="full">
                <div className="p-6 space-y-6">
                    ...
                    <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
                        <FormContainer title="Sales Transaction Header" icon={Database}>
                        ...
                        <FormContainer title="Item Identification" icon={ShoppingBag}>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <FormField label="Barcode" id="Barcode" icon={Barcode} value={form.Barcode} onChange={(e) => set('Barcode', e.target.value)} required />
                    ...
                    <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
                        <FormContainer title="Quantity Information" icon={Calculator}>
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                <FormField label="Unit" id="UNIT" value={form.UNIT} onChange={(e) => set('UNIT', e.target.value)} required />
                                <FormField label="Quantity" id="Qty" type="number" value={form.Qty} onChange={(e) => set('Qty', e.target.value)} required />
                        ...
                        <FormContainer title="Sales Pricing & Rates" icon={DollarSign}>
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
                        <FormContainer title="Discounts & Vouchers" icon={Percent}>
                            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                                ...
                                <FormField label="Customer Free" id="CUS_FREE" type="number" value={form.CUS_FREE} onChange={(e) => set('CUS_FREE', e.target.value)} />
                                <FormField label="Social Free" id="SOC_FREE" type="number" value={form.SOC_FREE} onChange={(e) => set('SOC_FREE', e.target.value)} />
                            </div>
                        </FormContainer>

                        <FormContainer title="Transaction Totals">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <FormField label="Net Sales Amt" id="NET_AMT" type="number" className="font-bold text-[#0097A7] bg-cyan-50" value={form.NET_AMT} onChange={(e) => set('NET_AMT', e.target.value)} required />
                            </div>
                        </FormContainer>
                    </div>

                    {/* Tax Breakdown */}
                    <FormContainer title="GST & Taxation Configuration">
                        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                            ...
                            <FormField label="Total Tax Amt" id="TOTAL_TAX" type="number" value={form.TOTAL_TAX} onChange={(e) => set('TOTAL_TAX', e.target.value)} required />
...
export default QuotationSalesTran;
